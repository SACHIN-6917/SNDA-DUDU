...
function initPandaBot() {
    ...
    // --- Suggestions Logic ---
    const suggestionsContainer = document.getElementById('pandaSuggestions');
    const suggestions = [
        { text: "🏭 Top Visits", query: "What are the best industrial visits?" },
        { text: "💰 Payment", query: "How to pay?" },
        { text: "🔑 Admin Login", query: "How to login as admin?" },
        { text: "📞 Contact", query: "Contact support" }
    ];
    ...
    function renderSuggestions() {
        ...
        suggestions.forEach(s => {
            ...
            chip.addEventListener('click', () => sendMessage(s.query));
    ...
    async function sendMessage(manualQuery = null) {
        const q = manualQuery || (pandaInput.value || '').trim();
        if (!q) return;
